...
std::unique_ptr<AIInterface>
AIConfig::createModel(AIModelType type) {
  switch (type) {
    case AIModelType::L0:
...
std::string
AIConfig::getModelName(AIModelType type) {
  switch (type) {
    case AIModelType::L0:
      return "L0";
    case AIModelType::L1:
...
std::string
AIConfig::getModelDescription(AIModelType type) {
  switch (type) {
    case AIModelType::L0:
...
AIModelType
AIConfig::parseModelType(const std::string& name) {
  if (name == "L0" || name == "l0") {
